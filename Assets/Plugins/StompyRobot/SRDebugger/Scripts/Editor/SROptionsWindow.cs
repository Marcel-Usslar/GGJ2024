...
namespace SRDebugger.Editor
{
    ...
    class SROptionsWindow : EditorWindow
    {
        ...
        private static readonly Regex _colorTagRegex = new Regex(@"<color=""?(#[0-9a-f]*""?)>([^<]*)<\/color>", 
        ...
        void OnGUI()
        {
            ...
            foreach (var kv in _options)
            {
                ...
                var hasColorTag = _colorTagRegex.IsMatch(kv.Key);
                var matches = _colorTagRegex.Match(kv.Key).Groups;
                
                var categoryName = hasColorTag ? matches[2].Value : kv.Key;
                var normalColor = _foldout.normal.textColor;

                if (hasColorTag)
                {
                    var colorHex = matches[1].Value;
                    
                    if (!EditorGUIUtility.isProSkin)
                        colorHex = CategoryColorsMap.ToLightModeColor(colorHex);
                    
                    ColorUtility.TryParseHtmlString(colorHex, out var color);
                    SetColor(_foldout, color);
                }

                state.IsOpen = EditorGUILayout.Foldout(state.IsOpen, categoryName, _foldout);
                SetColor(_foldout, normalColor);
        ...
        private void SetColor(GUIStyle guiStyle, Color color)
        {
